...
public class JPAGenerator implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws Exception {
		...
		try {
			...
			File schemaFile = new File(getClass().getClassLoader().getResource(XML_SCHEMA_FILE).getFile());
		...
		if (jpa!=null){		
			...
			for (DomainObjectComplexType domainObject : jpa.getDomainObject()){
				...
				if (domainObject.getEntity()!=null){
					...
					try {
						...
						logger.info("Se crea la clase de entidad correspondiente al objeto de dominio {} en la ruta {}", domainObject.getName(), entityPath);
						...
						logger.error("Hubo un error generando la clase de entidad correspondiente al objeto de dominio {}", domainObject.getName());
					} finally {
				...
				if (domainObject.getRepository()!=null){
					...
					try {
						String repositoryPath = fullPath + "\\repository\\" + domainObject.getName() + "Repository.java";
						logger.info("Se crea la clase de respositorio correspondiente al objeto de dominio {} en la ruta {}", domainObject.getName(), repositoryPath);
						...
						logger.error("Hubo un error generando la clase de respositorio correspondiente al objeto de dominio {}", domainObject.getName());
					} finally {
			...
			logger.info("Se han procesado un total de {} objetos de dominio, "
					+ "con un resultado de {} clases de entidad y {} clases de repositorio", jpa.getDomainObject().size(), entityClasses, repositoryClasses);
